...
💰 Send /plan to view subscription options"""
    ...
    if data.startswith("sendfiles"):
        ...
        text = "<b>🎬 ʏᴏᴜʀ ғɪʟᴇ ɪꜱ ʀᴇᴀᴅʏ!</b>\n\n<b>👇 ᴄʟɪᴄᴋ ᴛʜᴇ ʙᴜᴛᴛᴏɴ ʙᴇʟᴏᴡ ᴛᴏ ɢᴇᴛ ʏᴏᴜʀ ғɪʟᴇ (ғʀᴇᴇ)</b>\n\n"
        if PREMIUM_AND_REFERAL_MODE == True:
            text += "━━━━━━━━━━━━━━━━━━━━\n<i>💎 ᴡᴀɴᴛ ᴅɪʀᴇᴄᴛ ғɪʟᴇꜱ ᴡɪᴛʜᴏᴜᴛ ᴀᴅꜱ? /plan</i>"
...
✅ Click the button below to verify now!"""
        ...
        for file in files:
            ...
            if CUSTOM_FILE_CAPTION:
                try:
                    ...
                    f_caption=f_caption
            if f_caption is None:
                f_caption = f"@anihubyt25 {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files1['file_name'].split()))}"
        ...
        if temp.SHORT.get(user)==None:
            ...
            if user not in ADMINS:
                ...
                await message.reply_text(
                    text=f"<b>⚠️ Invalid file link!</b>\n\n"
                         f"<b>Please search in our movie group to get files.</b>\n\n"
    ...
    # Check if user went through proper group flow (temp.SHORT is set)
    ...
    if not files_:
        ...
        try:
            ...
            f_caption = f"@anihubyt25 <code>{title}</code>"
            if CUSTOM_FILE_CAPTION:
                try:
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"@anihubyt25 {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files['file_name'].split()))}"
    if not await db.has_premium_access(message.from_user.id):
        if not await check_verification(client, message.from_user.id) and VERIFY == True:
            btn = [[
                InlineKeyboardButton("ᴠᴇʀɪғʏ", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
